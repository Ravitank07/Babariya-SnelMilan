...
const Occupation = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/occupation/viewAll",
        {
          headers: {
  ...
  const handleSubmit = async () => {
    if (!inputValue) {
      alert("Please Enter Occupation First");
    } else if (inputValue && edit) {
      try {
        const response = await axios.put(
          `http://localhost:8000/api/occupation/update/${editId}`,
          {
            name: inputValue,
          },
          {
      ...
      try {
        const response = await axios.post(
          "http://localhost:8000/api/occupation/add",
          {
            name: inputValue,
          },
          {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8000/api/occupation/delete/${id}`, {
        headers: {
          Authorization: `${Authorization}`,
        },
      });
  ...
  return (
    <div className="occupation_main_container p-8">
      <div className="mt-12">
        <h1 className="text-[3rem]">Occupation</h1>
        ...
        <div className="flex items-center justify-between mt-5">
          <div>
            <input
              ...
              type="text"
              placeholder="Enter Occupation Here For Add"
              value={inputValue}
