...
const Taluka = () => {
  ...
  const handleSubmit = async () => {
    if (!inputValue) {
      alert("Please Enter Taluka First");
    } else if (inputValue && edit) {
      try {
        const response = await axios.put(
          `http://localhost:8000/api/taluka/update/${editId}`,
          {
            name: inputValue,
          },
          {
      ...
      try {
        const response = await axios.post(
          "http://localhost:8000/api/taluka/add",
          {
            name: inputValue,
          },
          {
        ...
        <div className="flex items-center justify-between mt-5">
          <div>
            <input
              ...
              type="text"
              placeholder="Enter Taluka Here For Add"
              value={inputValue}
              ...
              <tr key={i._id}>
                ...
                <td>
                  <button onClick={() => handleUpdate(i._id)}>
                    <CiEdit className="end_bttn" />
                  </button>
                </td>
                <td>
                  <button onClick={() => handleDelete(i._id)}>
                    <MdDelete className="end_bttn" />
                  </button>
                </td>
              </tr>
            ))}
